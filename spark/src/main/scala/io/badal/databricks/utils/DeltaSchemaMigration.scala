...
object DeltaSchemaMigration{
  def createTableIfDoesNotExist(tableName: String, schema: StructType)(implicit spark: SparkSession) = {
   ...
   * */
  def updateSchema(tableName: String, sourceSchema: StructType)(implicit spark: SparkSession): DeltaTable = {
    val emptyDF = spark.createDataFrame(spark.sparkContext.emptyRDD[Row], sourceSchema)
    emptyDF
      .write
