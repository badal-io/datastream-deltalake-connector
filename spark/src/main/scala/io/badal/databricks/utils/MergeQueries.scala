...
final case class MergeSettings(targetTableName: String,
                               idColName: String,
                               tsColName: String,
                               spark: SparkSession)

final case class MergeQueries(settings: MergeSettings) {
  ...
  def upsertToDelta(microBatchOutputDF: DataFrame, batchId: Long): Unit = {
    ...
    targetTable
      ...
      .merge(
        ...
        s"t.${idColName} = s.payload.$idColName"
      )
      .whenMatched("s.source_metadata.change_type = 'DELETE'")
      .delete()
      .whenMatched()
      .updateExpr(updateExp)
      .whenNotMatched("s.source_metadata.change_type != 'DELETE'")
